...
def get_top_three(prediction): #this function will be renamed to get_top_n and will take in an n arugment.
    ...
    if teams_by_wins[2].wins > teams_by_wins[3].wins:
        return [teams_by_wins[0].name,teams_by_wins[1].name,teams_by_wins[2].name]
    else:#tiebreaker
        ...
        # a good idea
        if teams_by_wins[1].wins > teams_by_wins[2].wins:
            out.append(teams_by_wins[0].name)
            out.append(teams_by_wins[1].name)
            #out.append(len(get_tie_breaker_teams(teams_by_wins,2)))
            for i in get_tie_breaker_teams(teams_by_wins,2):
                out.append(i.name);
            return out;
        elif teams_by_wins[0].wins > teams_by_wins[1].wins:
            out.append(teams_by_wins[0].name)
            #out.append(len(get_tie_breaker_teams(teams_by_wins,1)))
            for i in get_tie_breaker_teams(teams_by_wins,1):
                out.append(i.name);
            return out;
        else:
            for i in get_tie_breaker_teams(teams_by_wins,0):
                out.append(i.name);
            return out;
            
def get_tie_breaker_teams(teams_by_wins, num_teams_above_tie):
    ...
    for i in teams_by_wins:
        ...
        if i.wins == win_mark:
            tied_teams.add(i);
    return head_to_head_tiebreaker(tied_teams, 3-num_teams_above_tie)
...
def main():
    ...
    for i in d: #turn them into percentages rather than raw counts
        ...
        d[i] = int(1000*d[i]/SIMULATIONS) / 10;
